...
public class SquirrelPostProcessorProvider implements SquirrelComponent, SquirrelSingleton
{
	...
	private Map<Class<?>, SquirrelPostProcessor<?>> postProcessorRegistry = new ConcurrentHashMap<Class<?>, SquirrelPostProcessor<?>>();
	...
	public <T> void register(Class<T> componentClass, SquirrelPostProcessor<? super T> postProcessor)
	{
		...
		{
			SquirrelPostProcessor<? super T> existedProcessor = (SquirrelPostProcessor<? super T>) postProcessorRegistry
					.get( componentClass );
