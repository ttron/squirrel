...
public class DeferBoundActionBuilderImpl<T extends StateMachine<T, S, E, C>, S, E, C>
		implements DeferBoundActionBuilder<T, S, E, C>, DeferBoundActionFrom<T, S, E, C>, DeferBoundActionTo<T, S, E, C>,
		On<T, S, E, C>, SquirrelComponent
{
	...
	public void perform(Action<T, S, E, C> action)
	{
		...
		{
			deferBoundActionInfo.setActions( Collections.singletonList( warpConditionalAction( action ) ) );
		}
	}
	...
	{
		condition = FSM.newMvelCondition( expression, executionContext.getScriptManager() );
		return this;
	}
