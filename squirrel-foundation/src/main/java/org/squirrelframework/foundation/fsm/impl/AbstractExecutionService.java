...
public abstract class AbstractExecutionService<T extends StateMachine<T, S, E, C>, S, E, C> extends AbstractSubject
		implements ActionExecutionService<T, S, E, C>
{
	...
	protected final LinkedList<Pair<String, List<ActionContext<T, S, E, C>>>> actionBuckets = new LinkedList<Pair<String, List<ActionContext<T, S, E, C>>>>();
		...
		for ( int i = 0, actionSize = bucketActions.size(); i < actionSize; ++i )
		{
			...
			if (actionContext.action.weight() != Action.IGNORE_WEIGHT)
			{
				...
				catch (Exception e)
				{
					...
					Throwable t = (e instanceof SquirrelRuntimeException) ? ((SquirrelRuntimeException) e).getTargetException()
							: e;
					...
					TransitionException te = new TransitionException( t, ErrorCodes.FSM_TRANSITION_ERROR,
							new Object[] { actionContext.from, actionContext.to, actionContext.event, actionContext.context,
									actionContext.action.name(), e.getMessage() } );
			...
			{
				logger.info( "Method call action \"" + actionContext.action.name() + "\" (" + (i + 1) + " of " + actionSize
						+ ") was ignored." );
			}
		}
		...
		for ( Entry<ActionContext<T, S, E, C>, Future<?>> entry : futures.entrySet() )
		{
			...
			catch (Exception e)
			{
				...
				TransitionException te = new TransitionException( t, ErrorCodes.FSM_TRANSITION_ERROR,
						new Object[] { actionContext.from, actionContext.to, actionContext.event, actionContext.context,
								actionContext.action.name(), e.getMessage() } );
				fireEvent( new ExecActionExceptionEventImpl<T, S, E, C>( te, actionContext.position, actionTotalSize,
						actionContext ) );
				throw te;
			}
		}
			...
			extends AbstractExecActionEvent<T, S, E, C> implements BeforeExecActionEvent<T, S, E, C>
	{
		...
		static <T extends StateMachine<T, S, E, C>, S, E, C> BeforeExecActionEvent<T, S, E, C> get(int pos, int size,
				ActionContext<T, S, E, C> actionContext)
		{
			return new BeforeExecActionEventImpl<T, S, E, C>( pos, size, actionContext );
		}
	}

	static class AfterExecActionEventImpl<T extends StateMachine<T, S, E, C>, S, E, C> extends AbstractExecActionEvent<T, S, E, C>
			implements AfterExecActionEvent<T, S, E, C>
	{
		...
		static <T extends StateMachine<T, S, E, C>, S, E, C> AfterExecActionEvent<T, S, E, C> get(int pos, int size,
				ActionContext<T, S, E, C> actionContext)
		{
	...
	static class ActionContext<T extends StateMachine<T, S, E, C>, S, E, C>
	{
		...
		static <T extends StateMachine<T, S, E, C>, S, E, C> ActionContext<T, S, E, C> get(Action<T, S, E, C> action, S from,
				S to, E event, C context, T stateMachine, int position)
		{
