...
class StateImpl<T extends StateMachine<T, S, E, C>, S, E, C> implements MutableState<T, S, E, C>
{
			...
			List<ImmutableTransition<T, S, E, C>> allTransitions)
	{
		...
		{
			if (target == t || t.getCondition().getClass() == Conditions.Never.class)
				continue;
	...
	{
		this.entry( null, null, stateContext );
		final ImmutableState<T, S, E, C> lastActiveState = getLastActiveChildStateOf( this,
				stateContext.getStateMachineData().read() );
	...
	{
		entry( null, null, stateContext );
		return childInitialState != null ? childInitialState.enterShallow( stateContext ) : this;
	}


	@Override
	public void entry(ImmutableState<T, S, E, C> source, ImmutableState<T, S, E, C> target,
			final StateContext<T, S, E, C> stateContext)
	{
		...
		{
			stateContext.getExecutor().defer( entryAction, source.getStateId(), getStateId(), stateContext.getEvent(),
		...
		if (isParallelState())
		{
			...
			{
				parallelState.entry( source, target, stateContext );
	...
	public void exit(final StateContext<T, S, E, C> stateContext)
	{
		...
		{
			stateContext.getExecutor().defer( exitAction, getStateId(), null, stateContext.getEvent(), stateContext.getContext(),
	...
	public void internalFire(StateContext<T, S, E, C> stateContext)
	{
		...
		if (isParallelState())
		{
			...
			for ( ImmutableState<T, S, E, C> parallelState : parallelStates )
			{
				...
				TransitionResult<T, S, E, C> subTransitionResult = FSM.<T, S, E, C> newResult( false, parallelState,
						currentTransitionResult );
				StateContext<T, S, E, C> subStateContext = FSM.newStateContext( stateContext.getStateMachine(),
						stateContext.getStateMachineData(), parallelState, stateContext.getEvent(), stateContext.getContext(),
				...
				if (subTransitionResult.getTargetState().isFinalState())
				{
					...
					if (grandParentState != null && grandParentState.isParallelState())
					{
						...
						for ( ImmutableState<T, S, E, C> subState : getSubStatesOn( grandParentState,
								stateContext.getStateMachineData().read() ) )
						{
							if (!subState.isFinalState())
							{
		...
		for ( final ImmutableTransition<T, S, E, C> transition : transitions )
		{
			...
			if (currentTransitionResult.isAccepted())
			{
				...
				if (targetState.isFinalState() && !targetState.isRootState())
				{
					...
					AbstractStateMachine<T, S, E, C> abstractStateMachine = (AbstractStateMachine<T, S, E, C>) stateContext
							.getStateMachine();
					StateContext<T, S, E, C> finishContext = FSM.newStateContext( stateContext.getStateMachine(),
							stateContext.getStateMachineData(), parentState, abstractStateMachine.getFinishEvent(),
					...
					// stateMachine.fireImmediate(stateMachineImpl.getFinishEvent(),
		...
		if (currentTransitionResult.isDeclined() && getParentState() != null && !getParentState().isRegion()
				&& !getParentState().isParallelState())
		{
			...
			for ( ImmutableTransition<T, S, E, C> t : allTransitions )
			{
				...
				if (conflictTransition != null)
				{
					throw new RuntimeException(
							String.format( "Transition '%s' is conflicted with '%s'.", t, conflictTransition ) );
				}
			}
		}
	}
