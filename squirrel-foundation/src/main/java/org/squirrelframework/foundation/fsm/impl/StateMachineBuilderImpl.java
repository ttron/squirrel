...
public class StateMachineBuilderImpl<T extends StateMachine<T, S, E, C>, S, E, C> implements StateMachineBuilder<T, S, E, C>
{
	...
	private StateMachineBuilderImpl(Class<? extends T> stateMachineImplClazz, Class<S> stateClazz, Class<E> eventClazz,
			Class<C> contextClazz, Class<?>... extraParamTypes)
	{
		Preconditions.checkArgument( isInstantiableType( stateMachineImplClazz ),
				"The state machine class \"" + stateMachineImplClazz.getName() + "\" cannot be instantiated." );
		Preconditions.checkArgument( isStateMachineType( stateMachineImplClazz ), "The implementation class of state machine \""
				+ stateMachineImplClazz.getName() + "\" must be extended from AbstractStateMachine.class." );
		...
		methodCallParamTypes = contextInsensitive ? new Class<?>[] { this.stateClazz, this.stateClazz, this.eventClazz }
		...
		catch (Exception e1)
		{
			...
			catch (Exception e2)
			{
				throw new IllegalArgumentException(
						"Cannot find matched constructor for \'" + stateMachineImplClazz.getName() + "\'." );
			}
		}
	...
	{
		S from = "*".equals( transit.from() ) ? null : stateConverter.convertFromString( parseStateId( transit.from() ) );
		S to = "*".equals( transit.to() ) ? null : stateConverter.convertFromString( parseStateId( transit.to() ) );
		E event = "*".equals( transit.on() ) ? null : eventConverter.convertFromString( transit.on() );
		DeferBoundActionInfo<T, S, E, C> deferBoundActionInfo = new DeferBoundActionInfo<T, S, E, C>( from, to, event );
	...
	private void buildDeclareTransition(Transit transit)
	{
		...
		Preconditions.checkArgument( transit.type() != TransitionType.INTERNAL || transit.from().equals( transit.to() ),
		...
		if (states.get( fromState ) != null)
		{
			...
			for ( ImmutableTransition<T, S, E, C> t : theFromState.getAllTransitions() )
			{
				if (t.isMatch( fromState, toState, event, transit.priority(), transit.when(), transit.type() ))
				{
					...
					{
						Action<T, S, E, C> methodCallAction = FSM.newMethodCallActionProxy( callMethodExpression,
								executionContext );
						mutableTransition.addAction( methodCallAction );
					}
					return;
		...
		if (transit.type() == TransitionType.INTERNAL)
		{
			InternalTransitionBuilder<T, S, E, C> transitionBuilder = FSM.newInternalTransitionBuilder( states,
					transit.priority(), executionContext );
		...
		try
		{
			if (transit.when() != Conditions.Always.class)
			{
				...
				if (StringUtils.isNotEmpty( transit.whenMvel() ))
				{
	...
	{
		return (value != null && value.startsWith( "#" )) ? stateAliasToDescription.get( value.substring( 1 ) ) : value;
	}
	...
	private void rememberStateAlias(State state)
	{
		...
		{
			throw new RuntimeException( "Cannot define duplicate state alias \"" + state.alias() + "\" for state \""
					+ state.name() + "\" and " + stateAliasToDescription.get( state.alias() ) + "\"." );
		}
	}
	...
	private synchronized void prepare()
	{
		...
		{
			// 1. install all the declare states, states must be installed before installing transition and
			// extension methods
		...
		if (UntypedStateMachine.class.isAssignableFrom( stateMachineImplClazz ))
		{
			...
			for ( final MutableState<T, S, E, C> state : states.values() )
			{
				UntypedMutableState untypedState = (UntypedMutableState) Proxy.newProxyInstance(
						...
						new Class[] { UntypedMutableState.class, UntypedImmutableState.class }, new InvocationHandler()
						{
							@Override
	...
	private String[] getEntryExitStateMethodNames(ImmutableState<T, S, E, C> state, boolean isEntry)
	{
		...
		return new String[] { "before" + postfix,
				prefix + ((stateConverter != null && !state.isFinalState()) ? stateConverter.convertToString( state.getStateId() )
						: StringUtils.capitalize( state.toString() )),
				"after" + postfix };
	}
	...
	private String[] getTransitionMethodNames(ImmutableTransition<T, S, E, C> transition)
	{
		...
		String toStateName = (stateConverter != null && !toState.isFinalState())
				? stateConverter.convertToString( toState.getStateId() )
				: StringUtils.capitalize( toState.toString() );
		...
		return new String[] { "transitFrom" + fromStateName + "To" + toStateName + "On" + eventName + "When" + conditionName,
				"transitFrom" + fromStateName + "To" + toStateName + "On" + eventName,
				"transitFromAnyTo" + toStateName + "On" + eventName, "transitFrom" + fromStateName + "ToAnyOn" + eventName,
				"transitFrom" + fromStateName + "To" + toStateName, "on" + eventName };
	}
	...
	private void installExtensionMethods()
	{
		for ( MutableState<T, S, E, C> state : states.values() )
		{
			...
			{
				addStateEntryExitMethodCallAction( exitMethodCallCandidate, methodCallParamTypes, state, false );
			}
			...
			{
				addStateEntryExitMethodCallAction( entryMethodCallCandidate, methodCallParamTypes, state, true );
			}
		}
	}
	...
	public T newStateMachine(S initialStateId, StateMachineConfiguration configuration, Object... extraParams)
	{
		...
		{
			throw new IllegalArgumentException(
					getClass() + " cannot find Initial state \'" + initialStateId + "\' in state machine." );
		}
	...
	private T postProcessStateMachine(Class<T> clz, T component)
	{
		if (component != null)
		{
			List<SquirrelPostProcessor<? super T>> postProcessors = SquirrelPostProcessorProvider.getInstance()
					.getCallablePostProcessors( clz );
	...
	private void defineChildStatesOn(S parentStateId, StateCompositeType compositeType, HistoryType historyType,
			boolean ignoreInitialState, S... childStateIds)
	{
		checkState();
