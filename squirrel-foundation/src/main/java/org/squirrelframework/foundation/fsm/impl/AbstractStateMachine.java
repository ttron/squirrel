 ...
 */
public abstract class AbstractStateMachine<T extends StateMachine<T, S, E, C>, S, E, C> extends AbstractSubject
		implements StateMachine<T, S, E, C>
{

	private static final Logger logger = LoggerFactory.getLogger( AbstractStateMachine.class );

	private final ActionExecutionService<T, S, E, C> executor = SquirrelProvider.getInstance()
			.newInstance( new TypeReference<ActionExecutionService<T, S, E, C>>()
			{
			} );
			...
			ActionExecutionService<T, S, E, C> executionService, boolean isDataIsolateEnabled)
	{
		...
		try
		{
			...
			StateContext<T, S, E, C> stateContext = FSM.newStateContext( this, localData, fromState, event, context, result,
					executionService );
		...
		catch (Exception e)
		{
			...
			fireEvent( new TransitionExceptionEventImpl<T, S, E, C>( lastException, fromStateId, localData.read().currentState(),
	...
	private void fire(E event, C context, boolean insertAtFirst)
	{
		...
		if (isEntryPoint)
		{
			...
			if (isDelegatorModeEnabled && StateMachineContext.currentInstance() != this)
			{
	...
	{
		ActionExecutionService<T, S, E, C> dummyExecutor = SquirrelProvider.getInstance()
				.newInstance( new TypeReference<ActionExecutionService<T, S, E, C>>()
				{
				} );
	...
	private ImmutableState<T, S, E, C> resolveRawState(ImmutableState<T, S, E, C> rawState)
	{
		...
		{
			@SuppressWarnings("unchecked")
			T linkedStateMachine = (T) ((ImmutableLinkedState<T, S, E, C>) rawState).getLinkedStateMachine( getThis() );
	...
	private void entryAll(ImmutableState<T, S, E, C> origin, StateContext<T, S, E, C> stateContext)
	{
		...
		{
			state = stack.pop();
			state.entry( origin, state, stateContext );
		}
	}
			...
			ActionExecutionService<T, S, E, C> executionService)
	{
		...
		StateContext<T, S, E, C> stateContext = FSM.newStateContext( this, localData, initialRawState, getStartEvent(), context,
	...
	public synchronized void terminate(C context)
	{
		...
		StateContext<T, S, E, C> stateContext = FSM.newStateContext( this, data, data.read().currentRawState(),
				getTerminateEvent(), context, null, executor );
	...
	public StateMachineData.Reader<T, S, E, C> dumpSavedData()
	{
		...
		try
		{
			StateMachineData<T, S, E, C> savedData = FSM.newStateMachineData( data.read().originalStates() );
	...
	private void dumpLinkedStateFor(ImmutableState<T, S, E, C> rawState, StateMachineData.Writer<T, S, E, C> target)
	{
		...
		{
			ImmutableLinkedState<T, S, E, C> linkedRawState = (ImmutableLinkedState<T, S, E, C>) rawState;
			StateMachineData.Reader<? extends StateMachine<?, S, E, C>, S, E, C> linkStateData = linkedRawState
					.getLinkedStateMachine( getThis() ).dumpSavedData();
	...
	private Object newListenerMethodProxy(final Object listenTarget, final Method listenerMethod,
			final Class<?> listenerInterface, final String condition)
	{
		final String listenerMethodName = ReflectUtils.getStatic( ReflectUtils.getField( listenerInterface, "METHOD_NAME" ) )
				.toString();
					...
					if (method.getName().equals( listenerMethodName ))
					{
						if (args[0] instanceof TransitionEvent)
						{
							...
							if (args[0] instanceof ActionEvent)
							{
								...
								if (args[0] instanceof StartEvent || args[0] instanceof TerminateEvent)
								{
									@SuppressWarnings("unchecked")
								...
								else
								{
									throw new IllegalArgumentException(
											"Unable to recognize argument type " + args[0].getClass().getName() + "." );
								}
					}
					else
		...
		Object proxyListener = Proxy.newProxyInstance( StateMachine.class.getClassLoader(), implementedInterfaces,
				invocationHandler );
		return proxyListener;
	}


	private Object invokeStateMachineListenerMethod(final Object listenTarget, final Method listenerMethod,
			final String condition, final StateMachineEvent<T, S, E, C> event)
	{
	...
	private Object invokeActionListenerMethod(final Object listenTarget, final Method listenerMethod, final String condition,
			final ActionEvent<T, S, E, C> event)
	{
		...
		for ( Class<?> parameterType : parameterTypes )
		{
			if (!isSourceStateSet && parameterType.isAssignableFrom( typeOfState() ))
			{
				...
				if (!isTargetStateSet && parameterType.isAssignableFrom( typeOfState() ))
				{
					parameterValues.add( event.getTo() );
					isTargetStateSet = true;
				}
				else
					if (!isEventSet && parameterType.isAssignableFrom( typeOfEvent() ))
					{
						parameterValues.add( event.getEvent() );
						isEventSet = true;
					}
					else
						if (!isContextSet && parameterType.isAssignableFrom( typeOfContext() ))
						{
							...
							if (parameterType.isAssignableFrom( AbstractStateMachine.this.getClass() ))
							{
								...
								if (parameterType.isAssignableFrom( Action.class ))
								{
	...
	private Object invokeTransitionListenerMethod(final Object listenTarget, final Method listenerMethod, final String condition,
			final TransitionEvent<T, S, E, C> event)
	{
		...
		if (event instanceof TransitionCompleteEvent)
		{
			...
			if (event instanceof TransitionExceptionEvent)
			{
		...
		for ( Class<?> parameterType : parameterTypes )
		{
			if (!isSourceStateSet && parameterType.isAssignableFrom( typeOfState() ))
			{
				...
				if (!isTargetStateSet && event instanceof TransitionEndEvent && parameterType.isAssignableFrom( typeOfState() ))
				{
					parameterValues.add( ((TransitionEndEvent<T, S, E, C>) event).getTargetState() );
					isTargetStateSet = true;
				}
				else
					if (!isTargetStateSet && event instanceof TransitionCompleteEvent
							&& parameterType.isAssignableFrom( typeOfState() ))
					{
						...
						if (!isTargetStateSet && event instanceof TransitionExceptionEvent
								&& parameterType.isAssignableFrom( typeOfState() ))
						{
			...
			Class<? extends Annotation> annotationClass, Class<?> listenerClass, Class<?> eventClass)
	{
		...
		if (anno != null)
		{
			...
			if (methodField != null && Modifier.isStatic( methodField.getModifiers() ))
			{
				...
				Object proxyListener = newListenerMethodProxy( listenerMethodProvider, listenerMethod, listenerClass,
						whenCondition );
	...
	public void addDeclarativeListener(final Object listenerMethodProvider)
	{
		...
		{
			if ("--wait-notify-notifyAll-toString-equals-hashCode-getClass--".indexOf( "-" + listenerMethod.getName() + "-" ) > 0)
				continue;
			...
			for ( int i = 0; i < stateMachineListenerMapping.length; ++i )
			{
				registerDeclarativeListener( listenerMethodProvider, listenerMethod, this,
						(Class<? extends Annotation>) stateMachineListenerMapping[i][0], stateMachineListenerMapping[i][1],
						stateMachineListenerMapping[i][2] );
			}
			...
			for ( int i = 0; i < actionExecutorListenerMapping.length; ++i )
			{
				registerDeclarativeListener( listenerMethodProvider, listenerMethod, executor,
						(Class<? extends Annotation>) actionExecutorListenerMapping[i][0], actionExecutorListenerMapping[i][1],
						actionExecutorListenerMapping[i][2] );
			}
		}
	}
	...
	public static class StartEventImpl<T extends StateMachine<T, S, E, C>, S, E, C> extends AbstractStateMachineEvent<T, S, E, C>
			...
			extends AbstractTransitionEvent<T, S, E, C> implements StateMachine.TransitionExceptionEvent<T, S, E, C>
	{
		...
		public TransitionExceptionEventImpl(TransitionException e, S sourceState, S targetState, E event, C context,
				T stateMachine)
		{
			super( sourceState, event, context, stateMachine );
			this.targetState = targetState;
