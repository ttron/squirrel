...
class TransitionImpl<T extends StateMachine<T, S, E, C>, S, E, C> implements MutableTransition<T, S, E, C>
{
	...
	public ImmutableState<T, S, E, C> transit(final StateContext<T, S, E, C> stateContext)
	{
		...
		{
			stateContext.getExecutor().defer( action, sourceState.getStateId(), targetState.getStateId(), stateContext.getEvent(),
	...
	private void doTransit(ImmutableState<T, S, E, C> source, ImmutableState<T, S, E, C> target,
			StateContext<T, S, E, C> stateContext)
	{
		if (target.isChildStateOf( source ) && type == TransitionType.EXTERNAL)
		{
			...
			source.entry( source, target, stateContext );
		}
	 ...
	 */
	private void doTransitInternal(ImmutableState<T, S, E, C> source, ImmutableState<T, S, E, C> target,
			StateContext<T, S, E, C> stateContext)
	{
		if (source == this.getTargetState())
		{
			...
			else
			{
				source.exit( stateContext );
				transit( stateContext );
				getTargetState().entry( source, target, stateContext );
			}
		}
		else
			if (source == target)
			{
				...
				if (source.getParentState() == target.getParentState())
				{
					// Handles 4.
					// Handles 5a. after traversing the hierarchy until a common ancestor if found.
					source.exit( stateContext );
					transit( stateContext );
					target.entry( source, target, stateContext );
				}
				else
				{
	...
	{
		// Fix issue17
		if (type == TransitionType.INTERNAL && stateContext.getSourceState().getStateId() != targetState.getStateId())
		{
			return;
		}
	...
	{
		return sourceState + "-[" + event.toString() + ", " + condition.name() + ", " + priority + ", " + type + "]->"
				+ targetState;
	}
	...
	public void verify()
	{
		if (type == TransitionType.INTERNAL && sourceState != targetState)
		{
			throw new RuntimeException(
					String.format( "Internal transition source state '%s' " + "and target state '%s' must be same.", sourceState,
							targetState ) );
		}
	}
}
