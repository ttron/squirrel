...
public class StateMachinePerformanceMonitor
{
	...
	@OnTransitionEnd
	public void onTransitionEnd(Object sourceState, Object targetState, Object event, Object context,
			StateMachine<?, ?, ?, ?> fsm)
	{
		waitIfBusyStat();
		...
		synchronized (this)
		{
			if (maxTransitionConsumedTime.get( tKey ) == null || delta > maxTransitionConsumedTime.get( tKey ).get())
			{
				maxTransitionConsumedTime.put( tKey, new AtomicLong( delta ) );
			}
			if (minTransitionConsumedTime.get( tKey ) == null || delta < minTransitionConsumedTime.get( tKey ).get())
			{
	...
	public void onAfterActionExecuted(StateMachine<?, ?, ?, ?> fsm, Action<?, ?, ?, ?> action)
	{
		...
		synchronized (this)
		{
			if (maxActionConsumedTime.get( aKey ) == null || delta > maxActionConsumedTime.get( aKey ).get())
			{
				maxActionConsumedTime.put( aKey, new AtomicLong( delta ) );
			}
			if (minActionConsumedTime.get( aKey ) == null || delta < minActionConsumedTime.get( aKey ).get())
			{
	...
	public synchronized StateMachinePerformanceModel getPerfModel()
	{
		...
		float averageTranstionConsumedTime = totalTransitionConsumedTime / (totalTransitionInvokedTimes + Float.MIN_VALUE);
		...
		float averageActionConsumedTime = totalActionConsumedTime / (totalActionInvokedTimes + Float.MIN_VALUE);
