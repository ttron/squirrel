...
public class ReflectUtils
{
	...
	private static Method getMethod(Class<?> clazz, String methodName, Class<?>[] parameterTypes, Class<?> original)
	{
		...
		try
		{
			...
			if (logger.isTraceEnabled())
			{
				logger.trace( "found method " + getClassNameSafe( clazz ) + "." + methodName + "("
						+ Arrays.toString( parameterTypes ) + ")" );
			}
		}
		catch (SecurityException e)
		{
			throw new SquirrelRuntimeException( e, ErrorCodes.NOT_ALLOW_ACCESS_METHOD, getClassNameSafe( clazz ), methodName,
		...
		catch (NoSuchMethodException e)
		{
			...
			{
				throw new SquirrelRuntimeException( e, ErrorCodes.METHOD_NOT_FOUND, original.getName(), methodName,
						getParameterTypesText( parameterTypes ) );
			}
		}
		return method;
	...
	public static <T> Constructor<T> getConstructor(Class<T> type, Class<?>[] parameterTypes)
	{
		...
		{
			throw new SquirrelRuntimeException( e, ErrorCodes.CONSTRUCTOR_NOT_FOUND, type.getName(),
					Arrays.toString( parameterTypes ) );
		}
	}
	...
	private static <T> T newInstance(Class<T> clazz, Constructor<T> constructor, Object[] args)
	{
		...
		{
			logger.trace(
					"creating new instance for class '" + getClassNameSafe( clazz ) + "' with args " + Arrays.toString( args ) );
		}
		if (constructor == null)
		{
			if (logger.isTraceEnabled())
		...
		catch (Throwable t)
		{
			...
			throw new SquirrelRuntimeException( cause, ErrorCodes.CONSTRUCT_NEW_INSTANCE_ERROR, getClassNameSafe( clazz ),
	...
	public static Object invoke(Method method, Object target, Object[] args)
	{
		...
		catch (InvocationTargetException e)
		{
			...
			targetException.printStackTrace();// XXX
			throw new SquirrelRuntimeException( targetException, ErrorCodes.METHOD_INVOKE_ERROR, method, Arrays.toString( args ),
					target, targetException.getCause() );
		}
		catch (Exception e)
		{
			throw new SquirrelRuntimeException( e, ErrorCodes.METHOD_INVOKE_ERROR, method, Arrays.toString( args ), target,
					e.getMessage() );
		}
		finally
		{
